...
def c_time(file, csv_writers, dband1):
    ...
    # Initialize sensor_info variable to collect sensor messages
    sensor_info = ''
    ...
    try:
        ...
        for line in fn:
            ...
            if igc == 'B':
                ...
                if aglalt > maxaglalt:
                    maxaglalt = aglalt
                if ((start != 0) and (int(mopval or 0) > 300) and (mopflg == 0)):
                    mopflg = 1
                    smop.append(atime)
                    mslmop.append(mslalt)
                    aglmop.append(int(aglalt))
                if ((mopflg == 1) and (int(mopval or 0) < 50) and (int(mop or 0) > 0)):
                    mopflg = 0
                    smop.append(atime)
                    mslmop.append(mslalt)
                    aglmop.append(int(aglalt))
            ...
            if ((spd <= 15) and (aglalt <= 200) and (start != 0)):
                ...
                if (len(senl) > 0):
                    ...
                    sensor_info += msg + '\n'
                if (len(smop) > 0):
                    msg = f"{gtype} Motor noise registered by MOP sensor at t={smop} and {aglmop}AGL"
                    print(msg)
                    sensor_info += msg + '\n'
                print('\n\n')
                ...
                if flight_year not in csv_writers:
                    ...
                    csv_writer.writerow(['Date (MM/DD/YYYY)', 'File', 'Gtype', 'Flight Time', 'Start Time', 'End Time', 'Landing', 'Sensor Info'])
